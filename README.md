...
> 우리는 검사 기계를 만들지 않습니다. 이미 있는 기계의 측정 결과를 못 바꾸게 만듭니다.

## 현재 상태

이 저장소는 해커톤 1차 서류 심사 제출을 위해 **3일 안에 구현한 프로토타입**입니다.
본선 진출 시 7주 멘토링 기간 동안 다음을 검토·개선할 계획입니다.

- **라이선스**: 안전모 판정에 쓰는 Ultralytics YOLO는 AGPL-3.0입니다. 네트워크 서비스로
  제공할 경우 전체 소스 공개 또는 Enterprise 라이선스가 필요한데, 본 저장소는 이미
  Public으로 공개되어 있어 현재는 조건을 충족합니다.
- **안전화 탐지**: 공개된 안전화 전용 탐지 모델을 찾지 못해 색상 휴리스틱으로 대체해둔
  상태입니다 (`backend/vision.py` 참고). 정확도 개선이 필요합니다.
- 그 외 인프라(테스트넷 선택, DB, 배포 구성 등)는 3일이라는 제약 안에서 고른 조합이라,
  본선 진출 후 실사용 규모에 맞게 재검토할 수 있습니다.
...
| 공격 | 방어 |
|---|---|
| QR 재사용 (replay) | `usedNonces` 매핑으로 온체인 1회 소진 |
| 위조 기기로 결과 제출 | 사전 등록된 기기 주소만 서명 유효 (EIP-712 device attestation) |
| AI 모델 몰래 교체 | `modelHash`를 함께 앵커링 → 판정 재현 검증 가능 |
